...
int main( int argc, char** argv )
{
    ...
    CvCapture* capture = 0;
    int imageNumber = 0;
    char nextImage[20] = "igvcimg0000.jpeg";
    ...
    for(;;)
    {
        double ti = (double)(cvGetTickCount());
        IplImage* frame = 0;
        int i, c, ksize;

        frame = cvQueryFrame( capture );
//        frame = cvLoadImage(nextImage, CV_LOAD_IMAGE_COLOR);
        if( !frame )
            break;
        ...
        if( c == 'q' || c == 'Q' || (c & 255) == 27 )
            break;
        if(c == 'b' || c == 'B'){
            sprintf(nextImage,"igvcimg%04d.jpeg",--imageNumber);
        }
        else if(c == 'n' || c == 'N'){
            sprintf(nextImage,"igvcimg%04d.jpeg",++imageNumber);
        }
        ti = (double)(1000000.0*cvGetTickFrequency()/(cvGetTickCount()-ti));
        printf("Running At %f FPS %s\n", ti, nextImage);
